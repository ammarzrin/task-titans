...
- [ ] **Repositories**: Implement `AuthRepository` (Login/Signup) and `ProfileRepository` (Fetch profiles).
- [ ] **Login Screen**: Family email/password login.
- [ ] **Profile Selection**: Grid of avatars.
- [ ] **PIN Protection**: Modal for entering 4-digit PIN to access a profile.
- [ ] **State Management**: Implement `UserNotifier` to track the *current active profile*.

### Parent Dashboard ("Mission Control")
- [ ] **Dashboard Layout**: Tab navigation (Missions, Rewards, Profiles).
- [ ] **Mission List**: View all active/pending missions.
- [ ] **Create Mission**: FAB -> Modal form with Title, Difficulty (XP/Gold logic), and Assignee.
- [ ] **Approval System**: View "Pending" tasks -> Approve (Grant XP/Gold) or Reject.
